...
- conda create -n xxx python=3.6: 创建python3.6版本的虚拟环境，名称xxx
...
- self并不是关键字，只是一个用于接受对象的普通函数，理论上可以用任何一个普通变量名代替
...
- 受保护的封装  protected
    - 受保护的封装是将对象成员进行一定级别的封装，然后，在类中或者子类中可以进行访问，但是在外部不可以
